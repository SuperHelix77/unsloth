...
# Adapted from z-lab/dflash (MIT), with DFlare support added for Unsloth Studio.
# Original: https://github.com/z-lab/dflash/blob/main/dflash/model_mlx.py
#
# This module intentionally stays self-contained: it is imported only by the
# MLX worker, so the normal CPU/CUDA and llama.cpp paths do not acquire MLX
# imports or speculative-decoding state.
...
# DFlash's MLX README documents a hardware-specific limit: quantized target
# or draft matmuls get slower when the verification width exceeds five tokens.
# Keep this policy in one place so callers cannot accidentally reintroduce the
# slower default from a checkpoint's block_size metadata.
MLX_QUANTIZED_VERIFY_MAX_BLOCK_SIZE = 5
...
def mlx_model_uses_quantized_weights(model: Any) -> bool:
    """Return whether an MLX module tree contains quantized linear layers."""
    return mlx_model_quantization_bits(model) is not None


def mlx_model_quantization_bits(model: Any) -> int | None:
    """Return the first MLX linear quantization width in a module tree."""
    try:
    ...
    for module in modules:
        ...
        return int(bits) if bits is not None else 0
    return None


def resolve_speculative_block_size(
    configured: int | None,
    draft_default: int,
    *,
    quantized: bool,
) -> int:
    """Resolve a safe DFlash verify width for the active MLX matmul path."""
    block_size = int(draft_default if configured is None else configured)
    if block_size < 1:
        raise ValueError("speculative block_size must be positive")
    if quantized:
        block_size = min(block_size, MLX_QUANTIZED_VERIFY_MAX_BLOCK_SIZE)
    return block_size
    ...
    def make_cache(self, max_kv_size=None):
        ...
        for layer_type in self.config.layer_types:
            if layer_type == "sliding_attention":
                ...
                # mlx-lm's generic make_prompt_cache cannot forward
                # max_kv_size to a model-owned make_cache() method. The draft
                # therefore accepts it explicitly so its full-attention cache
                # rotates with the target cache in long conversations.
...
class DFlareAttention(DFlashAttention):
    """DFlare attention with separate context and draft K/V projections.

    DFlash shares K/V projections for the target hidden-state context and the
    masked draft stream. DFlare deliberately separates those two sources; the
    rest of the attention/cache contract remains identical, so the same MLX
    verification loop can serve both architectures.
    """
    ...
    def __call__(self, x, x_ctx, rope, cache):
        ...
        queries = self.q_proj(x)
        ctx_keys = self.k_proj_target(x_ctx)
        ctx_values = self.v_proj_target(x_ctx)
        prop_keys = self.k_proj(x)
        prop_values = self.v_proj(x)
        queries = self.q_norm(
        ...
        ctx_keys = self.k_norm(
            ctx_keys.reshape(B, S, self.n_kv_heads, -1)
        ).transpose(0, 2, 1, 3)
        ctx_values = ctx_values.reshape(B, S, self.n_kv_heads, -1).transpose(0, 2, 1, 3)
        prop_keys = self.k_norm(
            prop_keys.reshape(B, L, self.n_kv_heads, -1)
        ).transpose(0, 2, 1, 3)
        prop_values = prop_values.reshape(B, L, self.n_kv_heads, -1).transpose(0, 2, 1, 3)
...
class DFlareDecoderLayer(DFlashDecoderLayer):
    def __init__(self, config: DFlashConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = DFlareAttention(config, layer_idx)


class DFlareDraftModel(DFlashDraftModel):
    """DFlare draft model with per-draft-layer target fusion."""

    layer_class = DFlareDecoderLayer

    def __init__(self, config: DFlashConfig):
        super().__init__(config)
        # The DFlash base creates one concatenating projection (``fc``). DFlare
        # keeps target taps independent and learns one fusion distribution per
        # draft layer instead.
        # MLX modules keep child modules in an internal tree, so removing the
        # Python attribute alone does not remove it from ``parameters()``.
        self.fc = None
        target_count = len(config.target_layer_ids)
        self.layer_fusion_weights = mx.zeros(
            (config.num_hidden_layers, target_count), dtype=mx.float32
        )
        # This is an inference-only cache. Bypass nn.Module registration so the
        # derived tensor is not treated as a second trainable parameter or
        # written into a checkpoint.
        object.__setattr__(self, "_fusion_weights_cache", None)
    ...
    def hidden_states(
        ...
        if target_hidden.ndim != 4:
            raise ValueError(
                "DFlare requires target hidden taps with shape [batch, seq, taps, hidden]"
            )
        if target_hidden.shape[2] != self.layer_fusion_weights.shape[1]:
            raise ValueError(
                "DFlare target tap count does not match the draft checkpoint: "
                f"got {target_hidden.shape[2]}, expected {self.layer_fusion_weights.shape[1]}"
            )
        h = self.embed_tokens(inputs) * self.embed_scale
        fusion = self._fusion_weights_cache
        if fusion is None:
            fusion = mx.softmax(self.layer_fusion_weights, axis=-1)
            object.__setattr__(self, "_fusion_weights_cache", fusion)
        for layer_idx, (layer, c) in enumerate(zip(self.layers, cache)):
            weights = fusion[layer_idx].astype(target_hidden.dtype)
            h_ctx = mx.sum(
                target_hidden * weights.reshape(1, 1, -1, 1), axis=2
            )
            h_ctx = self.hidden_norm(h_ctx)
            h = layer(h, h_ctx, self.rope, c)
        if logits_start:
            h = h[:, logits_start:]
        return self.norm(h)
...
def load_draft(
    ...
    local_path = Path(draft_id).expanduser()
    path = (
        local_path
    ...
    config = DFlashConfig(
        ...
        model_arch=str(
            dflash.get("model_arch")
            or cfg.get("model_arch")
            or (
                "dflare"
                if any("dflare" in str(a).lower() for a in (cfg.get("architectures") or []))
                else "dflash"
            )
        ).lower(),
    )
    weights = {k: v for f in path.glob("*.safetensors") for k, v in mx.load(str(f)).items()}
    architectures = tuple(str(a) for a in (cfg.get("architectures") or []))
    if config.model_arch == "dflare":
        model_class = DFlareDraftModel
...
def _target_hidden(model, *, stacked: bool):
    hidden_states = model._hidden_states
    if any(hidden is None for hidden in hidden_states):
        raise RuntimeError("MLX target did not expose every configured speculative hidden tap")
    if stacked:
        return mx.stack(hidden_states, axis=2)
    return mx.concatenate(hidden_states, axis=-1)


def _prefill_target(model, prompt, cache, hidden_limit, step_size, *, stacked=False):
    if step_size <= 0:
    ...
    while start < prompt.size:
        ...
        hidden = _target_hidden(model, stacked=stacked)
...
def _stream_generate(
    ...
    block_size = resolve_speculative_block_size(
        block_size,
        int(draft.config.block_size),
        quantized=(
            mlx_model_uses_quantized_weights(model)
            or mlx_model_uses_quantized_weights(draft)
        ),
    )
    sampler = make_sampler(temperature, top_p, top_k)
    stacked_hidden = isinstance(draft, DFlareDraftModel)
    ...
    # Keep speculative decoding on the same bounded/rotating KV policy as
    # ordinary MLX generation. Without this explicit argument, mlx-lm falls
    # back to its runtime default (8K in older bundled builds), so a long chat
    # silently loses the app's durable 64K setting as soon as DFlash is used.
    if max_kv_size is None:
        target_cache = make_prompt_cache(model)
        draft_cache = make_prompt_cache(draft)
    else:
        target_cache = make_prompt_cache(model, max_kv_size=max_kv_size)
        if hasattr(draft, "make_cache"):
            draft_cache = draft.make_cache(max_kv_size=max_kv_size)
        else:
            draft_cache = make_prompt_cache(draft, max_kv_size=max_kv_size)
    draft.bind(model)
    ...
    try:
        ...
        while n < max_tokens:
            ...
            with mx.stream(generation_stream):
                ...
                hidden = _target_hidden(model, stacked=stacked_hidden)
                if temperature > 0:
